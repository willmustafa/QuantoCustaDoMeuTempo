...
window.onload = function () {
  chrome.storage.sync.get(["salario"]).then((result) => {
    ...
    const element = document.querySelector(siteClasses[currentTab].price);
    if (!element) return;

    const valorHora = salarioInput / (8 * 5 * 4);
    const valorProduto = clearMoney(element.innerHTML);
    const valorFinal = Math.ceil(valorProduto / valorHora);

    if (
      currentTab == "www.mercadolivre.com.br" ||
      currentTab == "produto.mercadolivre.com.br" ||
      currentTab == "www.shopee.com.br"
    ) {
      const valorProduto = clearMoney(element.innerHTML) * 100;
      const valorFinal = Math.ceil(valorProduto / valorHora);
